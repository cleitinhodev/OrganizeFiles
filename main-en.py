...
def organizar_arquivos_tipo(pasta):
    ...
    print(f"Total files: {total_arquivos}")
    print(f"Moved files: {contador_arquivos}")
    ...
    for arquivo in os.listdir(pasta):
        ...
        if os.path.isfile(caminho_arquivo):
            ...
            if primeiro_caractere.isalpha():  # Se for uma letra
                ...
                pasta_destino = os.path.join(pasta, 'Others')  # Pastas para símbolos ou caracteres especiais
...
def organizar():
    ...
    if pasta == "" or pasta == "Select path":
        resultado_label.configure(text="Error: No option selected!", fg_color="red")
        return

    if tipo_var.get():
        organizar_arquivos_tipo(pasta)
        resultado_label.configure(text=f"Organized files!", fg_color="green")
    elif data_var.get():
...
def abrir_link():
    url = "https://www.bugzinho.com/organizefiles"
    webbrowser.open(url)
...
caminho_var = ctk.StringVar(value="Select path")
caminho_entry = ctk.CTkEntry(root, textvariable=caminho_var, width=305,
                             placeholder_text="Select a folder", bg_color='#151515')
caminho_entry.place(x=50, y=30)

# Botão para selecionar o caminho
selecao_btn = ctk.CTkButton(root, text="Search", command=selecionar, width=85,
...
tipo_checkbox = ctk.CTkCheckBox(root, text="Type", variable=tipo_var, command=lambda: marcar_uma_vez(tipo_var),
                                fg_color="#483d8b", hover_color="#2c226a", bg_color='#151515')
...
organizar_btn = ctk.CTkButton(root, text=" Organize", command=organizar,
